...
最新运行 (#003)：**全部 7 个场景通过，0 重试**（重启前端 dev server 后）。覆盖预约、改期、产品下单、管理员登录，断言贯穿 UI + API + DB；Stripe 全程 mock（后端 `STRIPE_FAKE_PAYMENTS` + 伪造签名 webhook），测试浏览器拦截 `stripe.com`，不调用真实外部服务。
